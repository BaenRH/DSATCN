...
def random_signal(signal, combin_num):  # combin_num扩充数据集（复制10倍）
...
def prepare_data(EEG_all, noise_all, combin_num, train_per, noise_type):
    ...
    validation_num = round(
        (EEG_all_random.shape[0] - train_num) / 2)  # the number of segmentations used in validation process
    # 0.8 0.1 0.1 train valid test
    ...
    SNR_train_dB = np.random.randint(-7,high=3,size=EEG_train.shape[0])
    ...
    for i in range(EEG_train.shape[0]):
        ...
        SNRs_train.append(SNR_train_dB[i])

    noiseEEG_train = np.array(noiseEEG_train)
    SNRs_train = np.array(SNRs_train)  # 保存训练集的SNR
    ...
    for i in range(noiseEEG_train.shape[0]):
        ...
        eeg_train_all_std = EEG_train[i] / np.std(noiseEEG_train[i])  # TODO
    ...
    print('validation data prepared, validation data shape: ', noiseEEG_val_end_standard.shape,
          EEG_val_end_standard.shape)
    ...
    for i in range(10):
        ...
        for j in range(eeg_test.shape[0]):
            ...
            SNRs_test.append(SNR_test_dB[i])
    ...
    # 保存数据
    save_dict1 = {
        "noiseEEG_train": noiseEEG_train_end_standard,
        "EEG_train": EEG_train_end_standard,
        "SNRs_train": SNRs_train}
    np.savez("../eegdenoisenet/traindata/train_eeg.npz", **save_dict1)

    save_dict2 = {
        "noiseEEG_val": noiseEEG_val_end_standard,
        "EEG_val": EEG_val_end_standard,
        "SNRs_val": SNRs_val}
    np.savez("../eegdenoisenet/valdata/val_eeg.npz", **save_dict2)

    save_dict3 = {
        "noiseEEG_test": noiseEEG_test_end_standard,
        "EEG_test": EEG_test_end_standard,
        "SNRs_test": SNRs_test}
    np.savez("../eegdenoisenet/testdata/test_eeg.npz", **save_dict3)
